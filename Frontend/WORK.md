...
| 5 | Code structure / maintainability | 10 | 5/10 | 5.0 | 🟠 userAPI split started (apiBase + authStorage + buildingManagerApi); pattern set for remaining domains |
...
| | **TOTAL** | **100** | | **🟢 74.1 / 100** | **CRA→Vite; security headers; ESLint; userAPI split started; tests** |
...
| ☐ | Task | Notes |
|---|------|-------|
| 🟡 | Split giant files | `userAPI.js` now uses `apiBase.js` (constants + 401 interceptor + `authHeaders`) + `authStorage.js` + `buildingManagerApi.js`; remaining domain groups split the same way (mechanical). React page files (`VehicleInsurance.js` etc.) best split against the running app. |
